...
이 책의 `labs/brain-archive`에는 이 파이프라인의 축소 구현이 들어 있다. 외부 LLM이나 embedding 없이 Daily Note의 H2 section을 읽고, action plan을 만들고, dry-run diff, graph doctor 리포트, 유사 노트 후보를 출력한다.
...
- similar search는 embedding 없이 토큰 벡터로 중복/관련 노트 후보를 계산한다.

실제 Agent를 만들 때는 이 lab의 토큰 벡터 검색을 embedding index로 바꾸고, confidence scoring과 approval queue를 추가하는 방향으로 확장한다.
