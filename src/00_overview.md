...
현재 원고는 1차 집필, 로컬 lab 구현, mdBook 검증, GitHub Pages workflow 구성, 최종 편집, 출판 전 QA를 마친 publish-ready 상태다. 책의 핵심 흐름인 `Daily Note -> Archive Agent -> dry-run diff -> apply -> graph doctor -> similar search -> report`는 본문과 lab으로 연결되어 있다.
