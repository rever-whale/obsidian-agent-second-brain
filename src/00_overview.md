...
이 책은 Obsidian을 사람이 직접 정리하는 개인 지식 관리 앱으로 쓰는 방법을 다루지 않는다. 핵심 목표는 Obsidian vault를 로컬 Markdown 지식 DB로 보고, AI Agent가 Daily Note를 읽어 노트를 분리하고, 링크를 제안하고, MOC를 갱신하고, 그래프 품질을 점검하는 운영 시스템을 설계하는 것이다.
...
- 실행 가능한 local lab: `labs/brain-archive`
- GitHub Pages publish workflow: `.github/workflows/deploy.yml`

## 현재 완료 상태

현재 원고는 1차 집필, local lab 구현, mdBook 검증, GitHub Pages workflow 구성을 마친 review 단계다. 책의 핵심 흐름인 `Daily Note -> Archive Agent -> dry-run diff -> apply -> report`는 본문과 lab으로 연결되어 있다.

아직 남은 작업은 다음 세 가지다.

- 최종 편집: 장 사이의 중복 표현을 줄이고 용어를 한 번 더 통일한다.
- Lab 확장 선택: semantic search와 graph doctor까지 실습 범위를 넓힐지 결정한다.
- 배포 확인: GitHub repository Settings에서 Pages Source가 `GitHub Actions`인지 확인하고, 원격 Actions 실행 결과를 확인한다.
