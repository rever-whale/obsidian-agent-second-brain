...
현재 원고는 1차 집필, 로컬 lab 구현, mdBook 검증, GitHub Pages workflow 구성, 최종 편집 1차를 마친 review 단계다. 책의 핵심 흐름인 `Daily Note -> Archive Agent -> dry-run diff -> apply -> graph doctor -> report`는 본문과 lab으로 연결되어 있다.

아직 남은 작업은 다음 두 가지다.

- Lab 확장 선택: semantic search까지 실습 범위를 넓힐지 결정한다.
