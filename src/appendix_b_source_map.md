# B. 소스 맵

이 부록은 각 장에서 사용할 주요 출처와 쓰임새를 정리한다.

| Source | Used for |
| --- | --- |
| Obsidian Internal links | Wikilink, heading link, block reference, alias 설계 |
| Obsidian Properties | YAML metadata와 Agent-readable schema |
| Obsidian Graph view | graph 품질 지표와 orphan/hub 개념 |
| Obsidian Bases | review dashboard와 query MOC |
| coddingtonbear/obsidian-local-rest-api | REST/MCP 기반 vault patch와 agent integration |
| jwhonce/obsidian-cli | CLI 기반 vault operation과 journal command |
| kepano/obsidian-skills | Agent skill layer와 Obsidian-flavored Markdown |
| brianpetro/obsidian-smart-connections | local embedding, semantic related notes, link suggestion |
| kartikkabadi/obsidian-vault-graph | vault graph CLI, backlinks, centrality, hygiene |
| Data-Wise/obsidian-cli-ops | PageRank, centrality, orphan/hub detection |
| blacksmithgu/obsidian-dataview | Markdown data index와 query dashboard |
| khoj-ai/khoj | AI Second Brain, custom agents, document search/RAG |

## 출처

- [Obsidian Help: Internal links](https://obsidian.md/help/links)
- [Obsidian Help: Properties](https://obsidian.md/help/properties)
- [Obsidian Help: Graph view](https://obsidian.md/help/plugins/graph)
- [Obsidian Help: Bases](https://obsidian.md/help/bases)
- [coddingtonbear/obsidian-local-rest-api](https://github.com/coddingtonbear/obsidian-local-rest-api)
- [jwhonce/obsidian-cli](https://github.com/jwhonce/obsidian-cli)
- [kepano/obsidian-skills](https://github.com/kepano/obsidian-skills)
- [brianpetro/obsidian-smart-connections](https://github.com/brianpetro/obsidian-smart-connections)
- [kartikkabadi/obsidian-vault-graph](https://github.com/kartikkabadi/obsidian-vault-graph)
- [Data-Wise/obsidian-cli-ops](https://github.com/data-wise/obsidian-cli-ops)
- [blacksmithgu/obsidian-dataview](https://github.com/blacksmithgu/obsidian-dataview)
- [khoj-ai/khoj](https://github.com/khoj-ai/khoj)
- [How People Manage Knowledge in their Second Brains](https://arxiv.org/abs/2509.20187)
