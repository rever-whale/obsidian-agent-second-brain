...
export function planArchive({ sections, sourcePath, vaultRoot }) {
  ...
  return sections.map((section, index) => {
    ...
    const target = targetForSection(section, kind, sourceDate);
    const meeting = kind === "create_meeting_note" ? meetingDetailsForSection(section, sourceDate) : null;
    const action = {
      ...
      target,
      title: titleForSection(section, kind, sourceDate),
      meeting,
...
function targetForSection(section, kind, sourceDate) {
  ...
  if (kind === "create_meeting_note") return meetingPathForSection(section, sourceDate);
...
function titleForSection(section, kind, sourceDate) {
  ...
  if (kind === "create_meeting_note") return meetingDetailsForSection(section, sourceDate).title;
...
function meetingPathForSection(section, sourceDate) {
  const meeting = meetingDetailsForSection(section, sourceDate);
  const fileParts = [meeting.date];
  if (meeting.timeSlug) fileParts.push(meeting.timeSlug);
  return `meetings/${slugify(meeting.category)}/${fileParts.join("-")}.md`;
}

function meetingDetailsForSection(section, sourceDate) {
  const subject = toMeetingTitle(sectionSubject(section));
  const match = subject.match(/^(\d{1,2}:\d{2})\s+(.+)$/);
  const date = sourceDate || "undated";
  if (!match) {
    return {
      date,
      time: "",
...
function renderNewNote(action) {
  return [
    ...
    ...renderMeetingProperties(action),
...
function renderActionMarkdown(action) {
  ...
  if (action.kind === "create_meeting_note") {
    return [
      "## Meeting Notes",
      "",
      `Source: [[${sourceBasename(action.source)}]]`,
      `Date: ${action.meeting?.date ?? action.sourceDate ?? "undated"}`,
      ...(action.meeting?.time ? [`Time: ${action.meeting.time}`] : []),
      "",
      action.text
    ].join("\n");
  }
...
function renderMeetingProperties(action) {
  if (action.kind !== "create_meeting_note" || !action.meeting) return [];
  return [
    `meeting_date: ${action.meeting.date}`,
    ...(action.meeting.time ? [`meeting_time: ${action.meeting.time}`] : []),
    `meeting_category: "${escapeYamlString(action.meeting.category)}"`
  ];
}
...
function projectIndexPathForParts(parts) {
  if (parts.length === 0) return "projects/untitled/untitled.md";
  return `projects/${parts.join("/")}/${parts.at(-1)}.md`;
}
...
function parseSubject(subject) {
  ...
  if (!project) {
    ...
    if (alias) {
      ...
      return {
        segments: [alias, ...tailSegments],
        pathParts: [slugify(alias), ...tailSegments.map((part) => slugify(part)).filter(Boolean)],
        linkedProjectDepth: 1
      };
    }
...
function parseProjectLink(target, alias = "") {
  ...
  const lastPart = projectParts.at(-1);
  const previousPart = projectParts.at(-2);
  if (lastPart === "index" || lastPart === previousPart) projectParts.pop();
...
function dateFromPath(sourcePath) {
  return path.basename(sourcePath).match(/\d{4}-\d{2}-\d{2}/)?.[0] ?? "";
}
